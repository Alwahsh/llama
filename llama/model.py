...
class FFF(nn.Module):
    ...
    def forward(self, x):
        ...
        orig_batch, orig_seq, orig_width = x.shape
        x = x.reshape(-1, orig_width)
        ...
        mlp2 = torch.bmm(mlp1.unsqueeze(1).to(torch.float16), selected_w2s.to(torch.float16)) 		# (batch_size, output_width)
        
        # done
        return mlp2.reshape(orig_batch, orig_seq, self.output_width)
